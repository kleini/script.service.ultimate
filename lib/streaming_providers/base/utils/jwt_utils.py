...
def decode_claims(jwt_token: str) -> Optional[Dict[str, Any]]:
    """
    Extract the raw claims dictionary without mapping.

    Args:
        jwt_token: JWT token string

    Returns:
        Dictionary of raw claims, or None if the token is not a decodable JWT.
    """
    try:
...
def get_expiry(jwt_token: str) -> Optional[float]:
    """
    Extract the "exp" claim from any JWT token.

    Args:
        jwt_token: JWT token string

    Returns:
        Expiry as a Unix timestamp, or None if the token carries no usable exp.
    """
    try:
